...
def filter_present_values(data: Dict[str, Optional[float]]) -> Dict[str, Optional[float]]:
...
if "auto_items" in st.session_state:
    ...
    visible_auto_items = filter_present_values(auto_items)

    st.subheader("4) Auto-Extracted Financial Fields")
    auto_items_df = pd.DataFrame(
        [{"field": FIELD_LABELS[k], "value": format_number(v)} for k, v in visible_auto_items.items()]
    )
    st.dataframe(auto_items_df, use_container_width=True)

    overview_col1, overview_col2, overview_col3 = st.columns(3)
    extracted_count = len(visible_auto_items)
    ...
    if enable_manual_correction:
        ...
        visible_final_items = filter_present_values(final_items)
        final_items_df = pd.DataFrame(
            [{"field": FIELD_LABELS[k], "value": format_number(v)} for k, v in visible_final_items.items()]
        )
        st.dataframe(final_items_df, use_container_width=True)

    ratios = calculate_ratios(final_items)
    visible_ratios = filter_present_values(ratios)
    ...
    ratio_df = pd.DataFrame(
        [{"ratio": k, "value": format_ratio(v)} for k, v in visible_ratios.items()]
    )
